...
@section('content')
	...
	<h1 style="display:inline">Teacher List</h1>

	<div class="float-right">
	<button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#addteachers">Add New Teacher</button>
	</div>

	<div>{{session('Pesan')}}</div>

	<!--Modal for adding new teachers-->
	<form method="post" action="/teachers/add">
		<div class="modal fade" id="addteachers" tabindex="-1" role="dialog">
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="addteachers">Add New Teacher</h5>
					...
					<div class="modal fade" id="edit-{{$s->id}}" tabindex="-1">
						<div class="modal-dialog modal-dialog-centered"  role="document">
							<div class="modal-content">
								...
								<div class="modal-body">
									<form action="/teachers/edit/{{ $s->id }}" method="POST">
										@method('patch')
										@csrf
										<div class="form-group">
											<label for="name">Name</label>
											<input type="text" name="name" value="{{ $s->teachers->name }}" class="form-control">
											@error('name')
											{{ $message }}
											@enderror
										</div>
										...
										<div class="form-group">
											<label for="birthdate">Birth Date</label>
											<input type="date" name="birthdate" value="{{ $s->teachers->birthdate }}" class="form-control">
											@error('birthdate')
											{{ $message }}
											@enderror
										</div>
										<div class="form-group">
											<label for="short_desc">Short Description</label>
											<input type="text" name="short_desc" value="{{ $s->teachers->short_description }}" class="form-control"> <br>
											@error('short_desc')
											{{ $message }} <br>
											@enderror
										</div>
					...
					<div class="modal fade" id="delete-{{$s->id}}" tabindex="-1">
						<div class="modal-dialog modal-dialog-centered" role="document">
							<div class="modal-content">
								...
								<div class="modal-footer">
									<button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
									<form action="/teachers/delete/{{$s->id}}" method="POST">
										@method('delete')
										@csrf
